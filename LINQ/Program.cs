    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var item in list.GroupBy(x => x.Type))
            {
                ...
                foreach (var c in item.Where(x => x.Price == (item.Min(y => y.Price)))) Console.WriteLine($"{c.Name} {c.Price:C}");
            }
        ...
        static List<Product> CreateList()
        {
            ...
            while (!reader.EndOfStream)
            {
                ...
                else list.Add(new Product { Id = values[0], Name = values[1], Quantity = int.Parse(values[2]), Price = Decimal.Parse(values[3]), Type = values[4] });
            }
            return list;
        }
    }
